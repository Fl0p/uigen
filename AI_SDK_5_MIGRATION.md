...
- [x] **INFO**: Files accessing message.content: src/lib/provider.ts, src/components/chat/MessageList.tsx (these will ALL need refactoring)

### 1.3 Assess Data Migration Needs
- [x] **ACTION**: Do you have existing message data in a database? (Yes/No): Yes
- [x] **ACTION**: If Yes, estimate number of stored messages: Unknown (uses Prisma with SQLite)
...
- [x] **ACTION**: Create a plan for addressing each FIXME in Phase 4
  - src/app/api/chat/route.ts: appendResponseMessages removed, need to manually merge messages
